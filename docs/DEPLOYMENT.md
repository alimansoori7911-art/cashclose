...
## افزودن مشتری جدید (چندفروشگاهی)

هر مشتری یک **زیردامنهٔ اختصاصی** می‌گیرد:

```
rahavi.cashclose.ir   →  فروشگاه رهاوی
refah.cashclose.ir    →  فروشگاه رفاه
```

کاربر هیچ‌گاه شناسه‌ای تایپ نمی‌کند و اصلاً نمی‌فهمد مشتری دیگری هم
وجود دارد. **فهرست مجموعه‌ها عمداً در فرم ورود نیست** — نام مشتریان یک
اطلاعات تجاری است و نباید در دسترس عموم باشد.

### گام ۱: رکورد مجموعه

```bash
docker compose exec db psql -U cashclose -d cashclose -c \
  "INSERT INTO tenants (id, name, slug, status, created_at, updated_at)
   VALUES (gen_random_uuid(), 'فروشگاه رفاه', 'refah', 'active', now(), now());"
```

`slug` همان زیردامنه است: فقط حروف کوچک انگلیسی، رقم و خط تیره.
...
یا یک‌بار برای همیشه با wildcard:

| نام | نوع | مقدار |
|---|---|---|
| `*` | A | آی‌پی سرور |

با wildcard، مشتری بعدی فقط به گام ۱ نیاز دارد.

### گام ۳: گواهی HTTPS

Caddy با این تنظیم برای همهٔ زیردامنه‌ها خودکار گواهی می‌گیرد:
...
### زیردامنه‌های رزروشده

این‌ها به مشتری داده نمی‌شوند چون با آدرس خود سرویس تداخل دارند:
`www`، `api`، `admin`، `app`، `mail`، `static`، `cdn`، `status`،
